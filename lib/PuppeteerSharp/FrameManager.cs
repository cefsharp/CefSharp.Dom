...
namespace CefSharp.Dom
{
    internal class FrameManager : IDisposable, IAsyncDisposable, IFrameProvider
    {
        ...
        internal FrameManager(DevToolsConnection client, IDevToolsContext devToolsContext, bool acceptInsecureCerts, TimeoutSettings timeoutSettings)
        {
            Connection = client;
            DevToolsContext = devToolsContext;
            _logger = Connection.LoggerFactory.CreateLogger<FrameManager>();
            NetworkManager = new NetworkManager(acceptInsecureCerts, this, Connection.LoggerFactory);
            TimeoutSettings = timeoutSettings;

            Connection.MessageReceived += Client_MessageReceived;
            Connection.Disconnected += (sender, e) => _ = OnClientDisconnectAsync();
        }
